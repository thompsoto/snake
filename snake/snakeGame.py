...
class Snake(object):
    ...
    def move(self, vX, vY):
        ...
        if len(self.body) == 2:  # If the Snake only has two squares
            secondX = self.body[1].posX  # Initial X position of tail
            secondY = self.body[1].posY  # Initial Y position of tail

        self.reorganize()  # Reorganizes the whole snake, leaving only the head to be changed

        if keys[pygame.K_UP]:  # If the "W" key is pressed, changes the head's direction upwards
        ...
        elif keys[pygame.K_LEFT]:  # If the "A" key is pressed, changes the head's direction to the left
            self.head.dirRight = 0
            self.head.dirLeft = 1
            self.head.dirDown = 0
            self.head.dirUp = 20
        ...
        elif keys[pygame.K_DOWN]:  # If the "S" key is pressed, changes the head's direction downwards
            self.head.dirRight = 0
            self.head.dirLeft = 0
            self.head.dirDown = 1
            self.head.dirUp = 0
        ...
        elif keys[pygame.K_RIGHT]:  # If the "D" key is pressed, changes the head's direction to the right
        ...
        if len(self.body) == 2:  # If the snake only has two parts
            if self.head.posX == secondX and self.head.posY == secondY:  # Checks if snake has backed into its tail
                flag = True
...
def redraw(window, snake, snack, score):
    ...
    font = pygame.font.Font("8-Bit Madness.ttf", 48)  # Font used to display score
    scoreText = font.render("SCORE: " + str(score), True, (255, 255, 255))  # Renders score text
    window.blit(scoreText, [10, 10])  # Blit's text to window
...
def gameOver(window, score):
    ...
    while run:
        ...
        font = pygame.font.Font("8-Bit Madness.ttf", 100)  # Font used to display score
        font2 = pygame.font.Font("8-Bit Madness.ttf", 48)  # Font used to display score
        font3 = pygame.font.Font("8-Bit Madness.ttf", 40)  # Font used to display score
        ...
        window.blit(gameOverText, [90, 250])  # Blit's text to window
        window.blit(scoreText, [215, 320])
        window.blit(restart, [70, 450])
...
def main():
    ...
    while run:
        ...
        if secondSquare:  # Checks if a Snake of length 2 has backed into itself (flag from snake.move())
            mixer.music.load("thud.mp3")  # Hit itself sound effect
            mixer.music.play()
            run = False

        if snakeHit(snake):  # Checks if Snake has collided with itself
            mixer.music.load("thud.mp3")  # Hit itself sound effect
            mixer.music.play()
            run = False
